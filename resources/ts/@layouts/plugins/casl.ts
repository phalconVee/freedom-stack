...
export const canNavigate = (to: RouteLocationNormalized) => {
  ...
  // If no route in the match has ACL meta, allow access (e.g. onboarding, dashboard)
  const hasAcl = to.matched.some(route => route.meta?.action != null && route.meta?.subject != null)
  if (!hasAcl)
    return true

  // Otherwise check if any matched route allows access
